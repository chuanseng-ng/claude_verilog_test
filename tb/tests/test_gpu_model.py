...
class TestGPUKernelModel:
    ...
    def test_thread_id_computation(self):
        ...
        tid = gpu._compute_thread_id(warp['block_id'], warp['warp_in_block'], 0)
        assert tid == (0, 0, 0)

        # Test last lane (thread 7)
        tid = gpu._compute_thread_id(warp['block_id'], warp['warp_in_block'], 7)
        assert tid == (7, 0, 0)
    ...
    def test_thread_id_2d_block(self):
        ...
        tid = gpu._compute_thread_id(warp['block_id'], warp['warp_in_block'], 4)
        assert tid == (0, 1, 0)
    ...
    def test_warp_scheduling_round_robin(self):
        ...
        scheduled = []
        original_schedule = gpu._schedule_warp
        ...
        gpu._schedule_warp = tracking_schedule
    ...
    def test_sign_extend(self):
        ...
        # Positive number
        assert gpu._sign_extend(0x00000001, 12) == 0x00000001

        # Negative number (bit 11 set)
        assert gpu._sign_extend(0x00000FFF, 12) == 0xFFFFFFFF
