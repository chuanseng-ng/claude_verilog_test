...
6. ✅ **SoC verification** (M9): boot 100/100; DMA+UART+SPI loopback; SW coherency (D$ flush→GPU→D$ inval); CPU-GPU IRQ-driven integration; DUT-side boot SRAM check. Found+fixed 2 RTL bugs (D-cache MMIO caching `go9`; axi4_crossbar AR/AW handshake+arbitration `7fs`). `soc_all` 73/73 at M9 (120/120 as of 2026-08-01, after the PMU epic).
