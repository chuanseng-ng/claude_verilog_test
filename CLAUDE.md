...
**Current Phase**: Phase 5 (SoC Integration) — ✅ COMPLETE (2026-06-24). M1–M12 done; M11 ASAP7 SoC P&R signed off at **571 MHz / 62.9 mW ⚠️ fabric-only, excludes both hard macros / 520×520 µm / 65.6 % util / 0 DRC / 0 antenna ⚠️ routing claims unsupported, see below** (run 14, sv2v frontend; PDN ⚠️ withdrawn — the grid is EMPTY, see below). Power-attribution root cause + bounds: `docs/PHASE5_RUN_HISTORY.md` §"Power figure caveat" (bead `ew3`). **The "0 DRC / 0 antenna" figure is vacuous — run 14's detailed routing committed zero wires (bead `xy6`); timing/power/area numbers are unaffected (GRT-estimated parasitics, as always) but every routing-clean claim is withdrawn.** `docs/PHASE5_RUN_HISTORY.md` §"Routing / physical-closure caveat". ⚠️ **The 571 MHz / 62.9 mW timing+power figure itself is now also unvalidated (bead `8f3`/`0p6`, 2026-09-16): it came from `OpenROAD.STAMidPNR-3` run WITHOUT an in-session `global_route`, which a CPU-block proof showed reports falsely-clean 0/0/0 violations on ~94% zero-wire parasitics. Run 14's artifacts are wiped (bead `alq`) and cannot be re-measured with the now-fixed `STA_POSTGRT_INSESSION_GRT=1`; the honest current-RTL replacement is 258.5 mW / setup WNS −2084 ps / 41,864 violators (`docs/PHASE5_RUN_HISTORY.md` §"Power figure caveat", bead `w3a`/`86a`/`0p6`) — a different, unfinished design point, not a re-run of run 14.** ✅ **UPDATE 2026-09-18 (bead `0d0`): the SoC has since been re-measured three times on the fixed flow; the current honest current-RTL point is setup WNS −727.37 ps / TNS −60,665 ps / 781 violators, hold CLEAN, 283.6 mW total / 89.6 mW fabric-only (`RUN_2026-09-17_20-41-11`, pinned OpenROAD 26Q2, annotation-gated, zero real signal drivers unannotated) — this supersedes the −2084 ps figure above as the current basis. Timing is still NOT closed at 1750 ps. Run 14 itself remains unvalidated and unrecoverable.** See `docs/PHASE5_RUN_HISTORY.md`.

**Status**: Phase 4 complete (2026-05-27, all GPU tests green; ASAP7 GPU sign-off 571 MHz / 262 mW ⚠️ STA-zero-wire, unvalidated post-`8f3`, re-run deferred — see `docs/GPU_ASAP7_RUN_HISTORY.md`); Phase 3 complete (2026-05-21, 139/139 total, ASAP7 1418 MHz sign-off ⚠️ same STA caveat, bead `8f3`/`0p6`); Phase 2 complete (2026-03-08, 75 MHz on Sky130, real post-route SPEF — unaffected).

🔴 **ASAP7 CPU/GPU macro netlists: PROVEN frontend miscompile (bead `u99` → `ma7`, 2026-09-19).** Both macros are built with `USE_SYNLIG:true`. A differential (same testbench/ROM/sequencing, Synlig gate netlist vs RTL sim) showed `rv32i_cpu_top`'s netlist returns **0x30** for an APB debug read of `DBG_GPR[4]` where RTL returns **0x0c** — harness error, timing races and debug-semantics explanations all excluded. Mechanism: `OPT_MUXTREE` mis-elaborating a **runtime-indexed mux**, the same class proven in beads `gcd` and `b0t`. `rtl/cpu/core/rv32i_regfile.sv` uses that idiom on **all three** read ports, so the two feeding ID/execute are **UNVERIFIED, not proven clean** (`u99` checked insn/PC retire order, not register values); `rtl/gpu/warp_scheduler.sv:152` is the same class, untested. Remedy (switch both configs to `USE_SYNLIG:false` + sv2v → re-harden both macros → SoC re-run, expect PPA to worsen as `valu_hls` did) is **not applied** and needs a human decision: bead `ma7` (P1). Treat every ASAP7 CPU/GPU macro PPA figure below as resting on a netlist from a frontend with a proven miscompile in this repo. **Sky130 and the SoC fabric are unaffected** (both use the sv2v frontend). 🔴 **UPDATE 2026-09-19 (bead `ma7` step 1): the corruption is PROVEN to reach the execute datapath, not just the debug port.** A branch-dependent differential (same `tb_cpu_macro_check.v` harness/method as `u99`, new program whose control flow depends on computed register values so an ID-stage regfile read corruption becomes observable via `commit_pc_o`, not the known-bad debug port) shows the two arms retire IDENTICALLY through a `BEQ x2,x3` instruction's own fetch (x2=x3=9, must be TAKEN) and then diverge exactly at that instruction's commit: the frontend-independent RTL reference (Verilator) takes the branch correctly (matches the Python golden model bit-exact); the same Synlig gate netlist u99 used evaluates it as NOT-TAKEN and parks at a dead marker instead. Exact coverage: proven corrupt for register indices x2/x3 (value 9) as read by the branch comparator's rs1/rs2 ports feeding EX; not proven for other indices, instruction types, or access patterns. Full method, evidence paths, and the read-port cone-sharing analysis: bead `ma7` notes and `docs/CPP_TO_RTL_HLS_EVALUATION.md`. 🔴 **UPDATE 2026-09-19 (bead `ma7` remedy, PARTIALLY APPLIED — CPU re-harden BLOCKED, GPU still deferred).** User-approved remedy applied to configs only so far: `pnr/asap7/cpu/config.json` + `config_3014.json` and `pnr/asap7/gpu/config.json` + `config_3014.json` are now `USE_SYNLIG:false` with a new sv2v-preprocessing Makefile pipeline (`asap7-cpu-sv2v`, `asap7-gpu-sv2v`), mirroring the already-applied `valu_hls` fix. **The CPU re-harden PD run has NOT completed cleanly**: 3 attempts (lint fix needed and applied; a known unbounded-resizer-loop defect, bead bpp, needed and got `PL_RESIZER_TIMING_MAX_PASSES=2`; the 3rd attempt, `RUN_2026-09-19_15-42-11`, reached `OpenROAD.DetailedRouting` cleanly through CTS/GRT/resizer but then hit a **new, unresolved routing-congestion regression** — 108716 DRC violations, ~53x this block's own best-ever prior routed baseline of 2045, concentrated as `DRT-0255` failures on SRAM macro pins — that leaves `OpenROAD.RCX` extracting zero parasitics and `OpenROAD.STAPostPNR` failing outright). Root cause NOT pinned down (ruled out: wrong OpenROAD binary, simple netlist-size growth); tracked as bead `lxv`. **`pnr/asap7/soc/macro/rv32i_cpu_top.*` are UNCHANGED and remain the old, Synlig-built, proven-corrupt views** — no macro view was regenerated from an incomplete run. GPU re-harden itself remains fully deferred (host-gated, unchanged from the note above); `pnr/asap7/soc/macro/gpu_top.*` also remain Synlig-built and suspect. See bead `ma7` notes for the full attempt history and the methodology-deviation discussion (disabling SPEF/MCSTA to force flow completion was considered and explicitly rejected as not matching this block's own established reference methodology). 🔴 **UPDATE 2026-09-19 (bead `lxv`, root cause narrowed further — CPU re-harden STILL BLOCKED).** Five single-variable experiments across two sessions have now excluded: `PL_MACRO_HALO`, `GRT_LAYER_ADJUSTMENTS`, a structural netlist difference at the failing `clk0`/`csb0`/`din0` pins (byte-identical gate types/fanin between the Synlig and sv2v builds), a placement-clearance difference around the SRAM macros (<1% delta), a genuine global-routing capacity problem (the failing run's own GRT reports 0/0/0 overflow at 11.86% usage), a relaxed `CLOCK_PERIOD` (780→1000 ps: DRC 109687 vs. 108716 baseline, despite −9.8% fewer resizer-inserted buffers), and a raised `PL_RESIZER_TIMING_MAX_PASSES` (2→4: DRC 109562 vs. 108716, despite +2.1% more buffers). DRC count is invariant to resizer-driven cell-count changes in either direction. The only unexcluded explanation is the SRAM hard-macro's own pin-access geometry (`sram_1rw_256x32_asap7.lef`: all ~74 signal pins `SHAPE ABUTMENT` on one edge) — pre-existing in both frontends, consistent with `e69`'s own non-zero 2045-violation baseline on identical geometry. A real fix likely requires a hard-macro LEF/pin-layout change, which **needs explicit human approval** before any macro view is touched; none was attempted. Full trail: bead `lxv`, `docs/ASAP7_RUN_HISTORY.md`.
...
`pnr/asap7/` is fully configured and signed off: `config.json` (705 ps / 1418 MHz, CTS clustering 8/10, density 50 %), `constraints/asap7.sdc` (single-period clock, hold/setup uncertainty 10/15 ps, SRAM ICG `gclk_sram` generated clock + false-paths), `macro_placement.cfg`, `pdn.tcl`, `sram_1rw_256x32_asap7_TT_0p7V_25C.lib` (RVT TT 0.7 V / 25 °C). Run with `make librelane-asap7`. Phase 2+3 PPA (no perf counters): **1418 MHz fmax, 27.27 mW power, 3 844 µm² stdcell, 0 DRC, 0 antenna ⚠️ unsupported — see bead `xy6`, +5.97 ps setup slack, +22.54 ps hold slack** at run `pnr/asap7/runs/RUN_2026-05-20_06-27-10/` (run directory no longer on disk to re-verify directly; `drt.tcl`'s catch-and-continue patch was already in place before this run per `docs/ASAP7_RUN_HISTORY.md`'s caveat banner, so timing/power/area stand but the DRC/antenna claim does not). **Phase 5 M7 update (2026-06-02):** adding the perf-monitoring CSRs lowers the standalone CPU sign-off to **1282 MHz (780 ps, +24 ps setup, +28 ps hold, 24.38 mW, 4 058 µm²; achievable ~1323 MHz)** at run `pnr/asap7/cpu/runs/RUN_2026-06-02_20-37-12/` — the 1418 MHz number does not hold with M7 (limiter = 836-fanout counter-write-enable cone). Moot for the SoC (GPU-governed ~571 MHz). `pnr/asap7/cpu/config.json` + `constraints/asap7.sdc` are now at 780 ps. See `docs/ASAP7_RUN_HISTORY.md` for the full campaign history. ⚠️ **Both the 1418 MHz and 1282 MHz figures above are `8f3`-affected** (bead `8f3`/`0p6`, 2026-09-16): stock `OpenROAD.STAMidPNR-3` without an in-session re-route, same zero-wire STA defect as the SoC. Run 43's own artifacts are gone (unrecoverable); the M7 1282 MHz point **was re-measured on current RTL with the fix** (`STA_POSTGRT_INSESSION_GRT=1`, run tag `bpp7_postgrt_set`) and does **not** hold — honest result is setup WNS **−169.7 ps / TNS −70,913 ps / 1008 violators** (hold clean, unaffected). Full detail: `docs/ASAP7_RUN_HISTORY.md`.
...
9. ✅ **Backend flow** (M11): full SoC synth + P&R + STA via `make librelane-asap7-soc` (sv2v frontend — Synlig can't synth this SoC; `pnr/asap7/soc/`, `phase5_soc.sdc` single-clock, `phase5_soc.upf` 4-domain). Signed off **571 MHz / 62.9 mW ⚠️ fabric-only, excludes both hard macros / 520×520 µm / 65.6 % util / 0 DRC / 0 antenna ⚠️ unsupported, see bead `xy6`** (run 14). PDN ⚠️ withdrawn — every ASAP7 run committed ZERO power-grid shapes (bead `4l8`); the "benign tap-cell artifact" reading was wrong. ⚠️ Timing/power also unvalidated post-`8f3` (STA zero-wire, run 14 artifacts wiped) — see `docs/PHASE5_RUN_HISTORY.md` §"Power figure caveat" bead `0p6` note. ✅ Current RTL HAS been re-measured on the fixed flow (bead `0d0`, 2026-09-18): −727.37 ps / 781 violators / hold clean / 283.6 mW at `RUN_2026-09-17_20-41-11`; run 14's own figures stay unvalidated. `docs/PHASE5_RUN_HISTORY.md`
10. ✅ **Sign-off** (M12): M11 PPA recorded; docs updated (CLAUDE/ROADMAP/PHASE_STATUS → Phase 5 complete). Indicative ASAP7 (predictive PDK). Follow-ups: sv2v↔RTL equivalence — largely done 2026-09-13 (bead `q7n`: flat `soc_top` 99.65 % proven / 0 disproven; 25 of 27 modules proven outright via `tools/verif/equiv_sv2v_module.sh`, bounded post-reset BMC passes on `pmu` and `dma_engine`; the SoC now elaborates in strict LRM mode after fixing `soc_top.sv` use-before-declare and two `__pnr__` shim `parameter string` mismatches — `soc_all` 183/183, re-sweep on the regenerated netlist unchanged) + CPU-macro burst-port LEF (`g0o`)
...
> **Note:** the warning below is deliberately OUTSIDE the `<!-- rtk-instructions -->`
> markers. That block is regenerated by `rtk init`, which would silently wipe anything
> placed inside it.
...
1. **False "identical" verdict.** A change consisting only of *modified* (similar) lines was
   ...
   line differed was called identical. Root cause (upstream PR #792): the identical-files guard
   checked `added == 0 && removed == 0` and ignored the `~modified` counter.
2. **Exit code always 0**, even when the diff was displayed correctly. This was the broader of the
   two — it affected *every* invocation, so `if diff a b; then` and `diff a b || fail` always took
   the "identical" branch while looking perfectly correct on screen.

Upstream: [#781](https://github.com/rtk-ai/rtk/issues/781),
[#2446](https://github.com/rtk-ai/rtk/issues/2446),
...
- The rewrite is cwd- and parse-dependent, so a bare `diff` can behave correctly in one context
  and be filtered in another. Intermittent, which is worse than consistently wrong.

Found via GH #119 (bead `hk9`): comparing two generated Verilog files to decide whether a code
generator's output was byte-reproducible. The filtered `diff` said "identical", which would have
recorded the exact opposite of the truth — the files differed by an embedded timestamp.
...
**Architecture in one line:** issues live in a local Dolt DB; `.beads/issues.jsonl` is an export of it. ⚠️ **There is no git-ref bead sync on this machine.** bd 1.0.4 has no `refs/dolt/data` mechanism at all — its backup model is `bd backup init <path>` pointed at a filesystem path or a DoltHub remote. The `refs/dolt/data` ref still present on `origin` is a fossil from a pre-1.0.4 bd, frozen at 2026-06-06; nothing advances it. `bd backup status`'s `Database size: 0 B` is a cosmetic reporting bug, not a broken backup (the real DB is 21 MB at `.beads/embeddeddolt/`).

**Backup destination (fixed 2026-09-09, bead `a7c`):** `bd backup init /nobackup/beads-backup` — **a different physical device** from the `/dev/sdb2` root that holds the database (since the 2026-09-15 drive reorg `/nobackup` is `/dev/sda2`; the old `/nobackup` and its backup were wiped and re-synced fresh). Auto-detection had previously chosen `.beads/backup/`, which is gitignored *and on the same disk as the DB*, so a single disk failure would have taken the database and every backup of it together. Verified: 9.7 MB synced, `bd backup status` reports `Destination: file:///nobackup/beads-backup`. **This survives a disk loss, not a machine loss** — and `/nobackup` is presumably not itself backed up. For machine-loss protection use a DoltHub remote instead. The tracked `issues.jsonl` remains the only bead state that reaches the git remote. Keep it tracked. It merges through a custom driver that regenerates it from the DB (`.gitattributes` → `tools/setup/beads_merge_driver.sh`, registered by `make setup-beads`); plain `union` merge silently reinstated stale `open` rows and reopened closed beads twice (bead `b1o`). There is exactly **one** Dolt DB, in the primary worktree; secondary worktrees have no `.beads/embeddeddolt` of their own. But the export hook writes to the **primary** worktree's `issues.jsonl` no matter which worktree the commit happens in, while a secondary worktree keeps its own checked-out copy frozen at branch-creation time. So an agent in a secondary worktree that stages `.beads/issues.jsonl` commits a stale snapshot — that, not any DB divergence, is where `b1o`'s stale rows came from. **Agents working in a secondary worktree should not `git add` `.beads/`.** See https://github.com/gastownhall/beads/blob/main/docs/SYNC_CONCEPTS.md for details and anti-patterns.
