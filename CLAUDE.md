...
9. ✅ **Backend flow** (M11): full SoC synth + P&R + STA via `make librelane-asap7-soc` (sv2v frontend — Synlig can't synth this SoC; `pnr/asap7/soc/`, `phase5_soc.sdc` single-clock, `phase5_soc.upf` 4-domain). Signed off **571 MHz / 62.9 mW ⚠️ fabric-only, excludes both hard macros / 520×520 µm / 65.6 % util / 0 DRC / 0 antenna ⚠️ unsupported, see bead `xy6`** (run 14). PDN = benign tap-cell artifact (CPU/GPU precedent). `docs/PHASE5_RUN_HISTORY.md`
